...
$sqlingreso=$conexion->query("SELECT tb_egreso_sindicato.*,tb_proveedores.nombres,tb_bancos.n_cuenta,tb_bancos.banco, tb_plan_subcuentas.descripcion as des FROM `tb_egreso_sindicato` inner join tb_proveedores on tb_egreso_sindicato.id_proveedor=tb_proveedores.id_proveedores inner join tb_bancos on tb_egreso_sindicato.id_banco=tb_bancos.id_banco inner join tb_plan_subcuentas on tb_egreso_sindicato.id_plan_subcuentas=tb_plan_subcuentas.id_plan_subcuentas where tb_egreso_sindicato.comprabante_n=".$id); 
...
while($consultasocio=mysqli_fetch_array($sqlingreso)){
  ...
  $pdf->setXY(30,45); 
  $pdf->MultiCell(150,5,$consultasocio['descripcion'].' '.$factur,0,'L',0); 
  // $pdf->setXY(175,110); 
