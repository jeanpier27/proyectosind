...
$fecha1=substr($fecha, 0, -13);
$fecha2=substr($fecha, 13);
    $sqlsocio=$conexion->query("select tb_ingreso_sindicato.*,tb_personas.nombre,tb_personas.apellido from tb_ingreso_sindicato inner join tb_personas on tb_ingreso_sindicato.id_persona=tb_personas.id_persona where tb_ingreso_sindicato.fecha>='".$fecha1."' and tb_ingreso_sindicato. fecha<='".$fecha2."'"); 
    ...
    <br><br><br><br><br>
                            ...
                            <?php 
                              if(isset($_GET['iding'])){ 
                                  $sqlupdatesind=$conexion->query("select *  from tb_ingreso_sindicato where id_ingreso_escuela=".$_GET['iding'] );
                        ...
                        $query="update tb_ingreso_sindicato set comprabante_banco='".$depos."',estado='".$estado."',observacion='".$observaciontotal."' where id_ingreso_escuela=".$id;
                        $a=$conexion->query($query);     
                        if($a){
                          echo '<script type="text/javascript">swal({title: "ok", text: "Actualizacion con exito...!", type: "success",  confirmButtonText: "Aceptar!",  closeOnConfirm: false},function(){  location.href="consultacompsindicato.php";});</script>';   
                        }else{
                          echo '<script type="text/javascript">swal("Error!", "No se pudo actualizar datos del socio!", "error")</script>';
                            ...
                            <table class="table table-hover table-bordered table-responsive order-table" aling="center" id="tabladatos">
                             ...
                             <th><a class="btn btn-info" onClick="window.open('comprobante_ingreso_sind.php?id=<?php echo($consultasocio['comprabante_n']); ?>', 'prueba', 'width=800, height=600')"><i class="glyphicon glyphicon-print"></i></a></th> 
                                <th><a class="btn btn-info" href="consultacompsindicato.php?fecha_consulta=<?php echo $fecha1.'+-+'.$fecha2?>&iding=<?php echo $consultasocio['id_ingreso_escuela']; ?>&consultar="><i class="glyphicon glyphicon-pencil"></i></a></th>
                            ...
                            <?php 
                              if(isset($_GET['ideg'])){ 
                                  $sqlupdatesindeg=$conexion->query("select *  from tb_egreso_sindicato where id_egreso_escuela=".$_GET['ideg'] );
                               ...
                               if(isset($_POST['guardareg'])){
                                ...
                                $observacionbd=$_POST['observacionbd'];
                                 $fecha=date('Y-m-d H:i:s');
                        $observaciontotal=$observacionbd.' ('.$fecha.' usuario: '.$_SESSION['nombres'].'.- '.$observacion.')';
                        $query="update tb_egreso_sindicato set estado='".$estado."',observacion='".$observaciontotal."' where id_ingreso_escuela=".$id;
                        $a=$conexion->query($query);     
                        if($a){
                          echo '<script type="text/javascript">swal({title: "ok", text: "Actualizacion con exito...!", type: "success",  confirmButtonText: "Aceptar!",  closeOnConfirm: false},function(){  location.href="consultacompsindicato.php";});</script>';   
                        }else{
                          echo '<script type="text/javascript">swal("Error!", "No se pudo actualizar datos del socio!", "error")</script>';
                            ...
                            <table class="table table-hover table-bordered table-responsive order-table" aling="center" id="tabladatos2">
                              ...
                              $sqlegreso=$conexion->query("select * from tb_egreso_sindicato where fecha>='".$fecha1."' and fecha<='".$fecha2."'");
                                while($consultasoci=mysqli_fetch_array($sqlegreso)){
                               ?>

                                <tr>
                                <th><a class="btn btn-info" onClick="window.open('comprobante_egreso_sindicato.php?id=<?php echo($consultasoci['comprabante_n']); ?>', 'prueba', 'width=800, height=600')"><i class="glyphicon glyphicon-print"></i></a></th> 
                                 <th><a class="btn btn-info" href="consultacompsindicato.php?fecha_consulta=<?php echo $fecha1.'+-+'.$fecha2?>&ideg=<?php echo $consultasoci['id_egreso_escuela']; ?>&consultar="><i class="glyphicon glyphicon-pencil"></i></a></th>
                               <?php $totalegre=$totalegre+$consultasoci['saldo']; ?>  
                                <td><?php echo($consultasoci['fecha']); ?></td>
                                  <?php 
                                  $consult=$conexion->query("select tb_detalle_egreso_sindi.*,tb_facturasxcobrar.n_factura_ntv,tb_facturasxcobrar.total,tb_facturasxcobrar.subtotal,tb_facturasxcobrar.sobtotal_c,tb_facturasxcobrar.porc_renta,tb_facturasxcobrar.valor_renta,tb_facturasxcobrar.valor_iva,tb_facturasxcobrar.porc_iva FROM `tb_detalle_egreso_sindi` inner join tb_facturasxcobrar on tb_detalle_egreso_sindi.id_facturasxcobrar=tb_facturasxcobrar.id_facturasxcobrar where tb_detalle_egreso_sindi.comp_egreso_sindicato=".$consultasoci['comprabante_n']);
                                  while($con=mysqli_fetch_array($consult)){
                                      $facturas=$facturas.' '.$con['n_factura_ntv'];
                                  }
