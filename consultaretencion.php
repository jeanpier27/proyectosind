     ...
     <div class="content-page-container full-reset custom-scroll-containers">
        ...
        <div class="container">
            <div class="page-header">
              <h1 class="all-tittles">SINDICATO DE CHOFERES  DE NARANJAL<small> - Consultar Retencion</small></h1>
            </div>
        </div>
     <div class="container-flat-form">
                    ...
                    <div class="row">
                       ...
                       <form method="GET" action="">
                            ...
                            <?php 
                            if(isset($_GET['id'])){
                                 $sqlupd=$conexion->query("select tb_facturasxcobrar.*,tb_proveedores.nombres from tb_facturasxcobrar inner join tb_proveedores on tb_facturasxcobrar.id_proveedores=tb_proveedores.id_proveedores where tb_facturasxcobrar.id_facturasxcobrar=".$_GET['id']); 
                        ...
                        <input type="hidden" name="id_person" value="<?php echo $_GET['id']; ?>">
                           ...
                           <input type="text" name="nombre" readonly class="letras" value="<?php echo $consultasocioup['n_retenc']; ?>">
                        ...
                        $observaciontotal=$observacionbd.' ('.$fecha.' usuario: '.$_SESSION['nombres'].'.- '.$observacion.')';
                              $sqlrete=$conexion->query("update tb_facturasxcobrar set estado ='".$estado."',observacion='".$observaciontotal."' where id_facturasxcobrar=".$id);
                              if($sqlrete){
                                echo '<script type="text/javascript">swal({title: "ok", text: "Actualizacion con exito...!", type: "success",  confirmButtonText: "Aceptar!",  closeOnConfirm: false},function(){  location.href="consultaretencion.php";});</script>';   
                              }else{
...
$fecha1=substr($fecha, 0, -13);
$fecha2=substr($fecha, 13);
$sqlsocio=$conexion->query("select tb_facturasxcobrar.*,tb_proveedores.nombres from tb_facturasxcobrar inner join tb_proveedores on tb_facturasxcobrar.id_proveedores=tb_proveedores.id_proveedores where tb_facturasxcobrar.fecha_fac1>='".$fecha1."' and tb_facturasxcobrar.fecha_fac1<='".$fecha2."' and fac_ntv<>'n' ");   
}else{
    $sqlsocio=$conexion->query("select tb_facturasxcobrar.*,tb_proveedores.nombres from tb_facturasxcobrar inner join tb_proveedores on tb_facturasxcobrar.id_proveedores=tb_proveedores.id_proveedores where fac_ntv<>'n'"); 
}
                            ...
                            <table class="table table-hover table-bordered table-responsive order-table" aling="center" id="tabladatos">
                              ...
                              <?php 
                               ...
                               ?>
                                ...
                                <tr>
                                <th><a class="btn btn-primary"  onClick="window.open('comprobante_retencion.php?id=<?php echo($consultasocio['n_retenc']); ?>', 'prueba', 'width=800, height=600')"><i class="glyphicon glyphicon-search"></i></a></th>
                               <th><a class="btn btn-info" href="consultaretencion.php?id=<?php echo $consultasocio['id_facturasxcobrar']; ?>"><i class="glyphicon glyphicon-pencil"></i></a></th>
                                <td><?php echo($consultasocio['id_facturasxcobrar']); ?></td>
                                <td><?php echo($consultasocio['n_retenc']); ?></td>
                                <td><?php echo($consultasocio['nombres']); ?></td>
                                <td><?php echo($consultasocio['valor_renta']); ?></td>
                                <td><?php echo($consultasocio['valor_iva']); ?></td>                                
